...
import hashimotonet.util.InputHandler;
...
public class UploadService {
    ...
    public boolean execute (HttpServletRequest request)
        ...
        // Base64クラスの宣言をする
        String base64 = InputHandler.is2String(request.getInputStream());
        log.info("base64=" + base64);

        // IDを取得する。
        // '@'文字はURLエンコードされているので、
        // 変換を行う。
        String identity = base64.substring(base64.indexOf("id=") + "id=".length(), base64.indexOf("base64=") - 1);
        identity = identity.replace("%40", "@");
    ...
    private static byte[] decode2Bytes(String encoded) {
    	String formatted = "";
    	if (encoded != null) {
    		if (encoded.indexOf("base64=") > 0) {
    			int start = encoded.indexOf("base64=") + "base64=".length();
